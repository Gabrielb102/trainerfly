...
const selectCategory = () => {
    ...
    // Emit Selected Category to parent so showListings can be set to true
...
<template>
    <div class="w-full h-fit min-h-20 flex flex-col items-center justify-center gap-2 p-4 aspect-square shadow-md/40 rounded-xl cursor-pointer bg-gray-100 hover:bg-gray-200 transition-colors"
        ...
        <div class="w-full h-full flex items-center justify-center">    
            <UIcon :name="category.icon ? `fa6-solid:${category.icon}` : 'fa6-solid:circle-question'" class="size-10 text-gray-400" />
        </div>
        <div class="w-full h-fit flex items-center justify-center gap-2">
            <span class="h-fit w-full text-md leading-5 font-bold text-center uppercase text-black">{{ category.name }}</span>
            <UButton v-if="category.has_children" @click.stop="viewChildCategories" class="rounded-full w-fit h-fit cursor-pointer" icon="fa6-solid:angle-right" variant="ghost" size="sm" />
        </div>
    </div>
</template>